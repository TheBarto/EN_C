...
void iniciar_GPIO(const char* canal, uint8_t direccion, uint8_t pull_up_down, uint16_t espera, uint8_t inicial)
{
	...
	{
		printf("Valor inv�lido para el modo del puerto GPIO. El estado debe ser PUD_UP (ALTO), PUD_DOWN (BAJO) o PUD_OFF (APAGADO).\n");
		return;
	}
	...
	{
		printf("Error al fijar la direcci�n del GPIO (ENTRADA o SALIDA). Falta un archivo o los permisos son insuficientes.\n");
		return;
	}
